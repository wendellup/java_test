 ...
 */
package io.nio.netty4_echo_server_udp.utils;

import io.nio.netty4_echo_server_udp.LocalUDPSocketProvider;
...
public class UDPUtils {
	...
	public static boolean send(byte[] fullProtocalBytes, int dataLen) {
		final DatagramSocket ds = LocalUDPSocketProvider.getInstance()
				.getLocalUDPSocket();
	...
	private static boolean send(DatagramSocket skt, byte[] d, int dataLen) {
		if (skt != null && d != null) {
			try {
				...
				// , InetAddress.getByName(ConfigEntity.serverIP),
				// ConfigEntity.serverUDPPort
						));
			} catch (Exception e) {
				Log.e(TAG,
						"【IMCORE】send方法中》》发送UDP数据报文时出错了：remoteIp="
								+ skt.getInetAddress() + ", remotePort="
								+ skt.getPort() + ".原因是：" + e.getMessage(), e);
				return false;
			}
		} else {
	...
	private static synchronized boolean send(DatagramSocket skt,
			DatagramPacket p) {
		...
		if ((skt != null) && (p != null)) {
			if (skt.isConnected())// 只有前面调用了connect方法后，isConnected才会返回true哦
			{
				try {
					...
					sendSucess = false;
					Log.e(TAG,
							"【IMCORE】send方法中》》发送UDP数据报文时出错了，原因是："
									+ e.getMessage(), e);
				}
			}
		} else {
			Log.w(TAG,
					"【IMCORE】在send()UDP数据报时没有成功执行，原因是：skt==null || p == null!");
		}
