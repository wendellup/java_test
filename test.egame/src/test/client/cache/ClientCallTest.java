...
import cn.egame.ext.op.AdvVersionManagerInfo;
...
public class ClientCallTest extends EGameClientBase {
	...
	public void testList() throws RemoteException {
		List<Integer> notFirstTagIds = EGameClientV2.getInstance()
				.listGameTagIdByGameTagType(0, 0, GameTagType.searchTag);
	...
	@Test
	public void listAdvPostionIdsByAppId() throws RemoteException {
		List<Integer> listAdvPostionIdsByAppId = getCacheList().getListInt(
				EGameCacheKeyV2.listAdvPostionIdsByAppId(746));
		System.out.println(listAdvPostionIdsByAppId);
		// [375, 374]
	}

	@Test
	public void listAdvByVersionRangeAndAppId() throws RemoteException {
		List<AdvDetail> advDetailList = EGameClientExtV2.getInstance()
				.listAdvByVersionRangeAndAppId(730964, 0, 400, 780);
		for (AdvDetail advDetail : advDetailList) {
			System.out.println(advDetail.getId());
		}
		// System.out.println(advDetailList.size());
	}
	
	@Test
	public void listAllAdvVersionManagerInfos() throws RemoteException {
		String cacheKey = EGameCacheKeyV2.listAllAdvVersionManagerInfos();
	    List<AdvVersionManagerInfo> infos = getCacheList().getListT(AdvVersionManagerInfo.class, cacheKey);
		for (AdvVersionManagerInfo advVersionManagerInfo : infos) {
			System.out.println(advVersionManagerInfo.getBusinessId());
		}
		// System.out.println(advDetailList.size());
	}
	...
	@Test
	public void listAdvIdsbyPositionId() throws RemoteException {
		List<Integer> listAdvIdsbyPositionId = getCacheList().getListInt(
				EGameCacheKeyV2.listAdvIdsbyPositionId(400));
		for (Integer advId : listAdvIdsbyPositionId) {
	...
	public void getParameterAppInfo() {
		...
		AppParameter appParameterInfo = getGameCache().getT(AppParameter.class,
				key);
	...
	public void listGIdBySortType() throws RemoteException {
		List<Integer> gameIds = getCacheList().getListInt(
				EGameCacheKeyV2
						.listGIdBySortType(SearchSortType.firstOnlineTime
								.getValue()));
	...
	public void getGameFileInfoById() throws RemoteException {
		GameFileInfo gameFileInfo = EGameClientV2.getInstance()
				.getGameFileInfoById(0, 0, 5003729);
	...
	public void pageOnlineGameServiceByAppId() throws ExceptionCommonBase {
		try {
			int total = 0;
			PageData pd = getGameServiceExt().pageOnlineGameServiceByAppId(0,
					0, 927, 0, 20);
			if (pd != null) {
				total = pd.getTotal();
				Map<String, List<OnlineGameServiceInfo>> map = (HashMap<String, List<OnlineGameServiceInfo>>) pd
						.getContent();
				if (map != null) {
					for (DateType dt : DateType.values()) {
						List<OnlineGameServiceInfo> list = map.get(dt
								.getValue());
	...
	public void listGameIdByTagId() throws ExceptionCommonBase {
		/*
		 * int tagId = 488390; List<Integer> gameIds =
		 * getCacheList().getListInt(EGameCacheKeyV2.listGameIdByTagId(tagId));
		 * int filterTagId = 488392; List<Integer> filterGameIds =
		 * getCacheList()
		 * .getListInt(EGameCacheKeyV2.listGameIdByTagId(filterTagId));
		 * System.out.println(gameIds); System.out.println(filterGameIds);
		 */
	...
	public void searchByName() throws RemoteException {
		String[] keyWords = new String[] { "跑酷", "捕鱼", "益智", "纪念碑谷", "僵尸",
				"小黄人", "我的世界", "消除", "摩托", "飞机", "益智,跑得快", "炮妹", "斗地主",
				"忍者,美女", "忍者", "麻将", "塔防", "塔防,益智", "果宝三国", "熊出没" };
		// 门户调用老搜索
		System.out.println("门户调用老搜索开始----------------->:");
		for (int i = 0; i < keyWords.length; i++) {
			List<Integer> list = EGameGameSearchClient.getInstance()
					.searchByName(keyWords[i], 1, 1105);
		...
		for (int i = 0; i < keyWords.length; i++) {
			PageData pd = EGameGameSearchClientBiz.getInstance().searchByName(
					0, 0, keyWords[i], 1, 0, 20, 1105, 1, null);
			if (pd == null) {
		...
		for (int i = 0; i < keyWords.length; i++) {
			PageData pd = EGameGameSearchClientBiz.getInstance()
					.getSuggestionKeyWord(0, 0, keyWords[i], 1, 0, 20,
							new ExtraInfo(MobileNetworkType.DIANXIN, 0, 762));
			if (pd == null) {
				System.out.println(keyWords[i] + ":" + 0);
			} else {
				System.out.println(keyWords[i] + ":" + pd.getTotal());
	...
	public static void _main(String[] args) {
		...
		messageInfo.setMsgBusinessType(MsgBusinessType.commentReplyMsg
				.getValue());
		UserMessageEvent userMessageEvent = new UserMessageEvent(messageInfo,
				611, MessageAction.add);
	...
	public void getMemoryByAdvertTypeAndIdV2() throws RemoteException {
		// MemoryAdvLink memoryAdvLink =
		// EGameClientExtV2.getInstance().getMemoryAdvLinkByMemoryId(0, 0,
		// 1079);
		Memory memoryInfo = EGameClientExt.getInstance()
				.getMemoryByAdvertTypeAndIdV2(730964, 0, AdvertType.game,
						(long) 0, "", 1, "");
	...
	public void hallFileTerminalLinkInfo() throws RemoteException {
		HallFileTerminalLinkInfo hallFileTerminalLinkInfo = EGameClientExtV2
				.getInstance().getHallFileInfoByTerminalIdAndVersionV2(730964,
						0, 1, "752", 8888007);
	...
	public void parameterApp() throws RemoteException {
		AppParameter app = EGameClientBiz.getInstance().getAppParameterById(0,
				0L, 11214);
	...
	public void testListGameFileIdByPackageNameAndStatus() {
		...
		try {
			gameFileInfoIds = EGameClientV2.getInstance()
					.listGameFileIdByPackageNameAndStatus(0, 0L, packageName,
							-1);
		} catch (RemoteException e) {
			e.printStackTrace();
		}
	...
	public void pageOnlineGameServiceByAppId2() throws RemoteException {
		PageData pd = EGameClientExt.getInstance()
				.pageOnlineGameServiceByAppId(0, 0L, 927, 0, 20);
		Map<String, List<OnlineGameServiceInfo>> map = (HashMap<String, List<OnlineGameServiceInfo>>) pd
				.getContent();
	...
	public void listCommentReplyMsgByUserId() throws IOException {
		...
		List<MessageInfo> messageInfos = (List<MessageInfo>) getGameListCache()
				.get(key);
			...
			pw = new PrintWriter(
					new OutputStreamWriter(
							new FileOutputStream(
									"E:\\git_github\\wendellup\\java_test\\test.egame\\out.txt"),
							"utf-8"));
			for (int gId = 1; gId < 5057868; gId++) {
				pw.println(gId);
				System.out.println(gId);
				List<Integer> gameIds = EGameClientV2.getInstance()
						.listSimilarityGidsByGameId(0, 0, gId);
	...
	public void getGameInfo() {
		try {
			GameInfo gameInfo = EGameClientBiz.getInstance()
					.getGameInfoByIdAndStatus(8888043, 0L, 730011, null);
			System.out.println(gameInfo);
		} catch (RemoteException e) {
			e.printStackTrace();
		}
	...
	public void sendMsg() {
		/*
		 * logger.info("111"); MessageInfo messageInfo = new MessageInfo(); long
		 * longMillis = System.currentTimeMillis()-1000*30;
		 * messageInfo.setInsertTime(longMillis);
		 * messageInfo.setEffectBeginTime(longMillis);
		 * messageInfo.setBusinessId(new Long(11).intValue());
		 * messageInfo.setMsgBusinessType
		 * (MsgBusinessType.commentReplyMsg.getValue()); UserMessageEvent
		 * userMessageEvent = new UserMessageEvent(messageInfo, 2,
		 * MessageAction.add); MessageClient.getInstance().pushMessageEvent(0,
		 * 2, userMessageEvent); logger.info("111");
		 */
		...
		messageInfo.setBusinessId(new Long(11).intValue());
		messageInfo
				.setMsgBusinessType(MsgBusinessType.commentTopMsg.getValue());
		UserMessageEvent userMessageEvent = new UserMessageEvent(messageInfo,
				811, MessageAction.add);
		MessageClient.getInstance().pushMessageEvent(0, 4, userMessageEvent);
		logger.info("111");
		try {
			Thread.sleep(10000);
	...
	public void getMsg() {
		// PageData pageData =
		// MessageClient.getInstance().pageMessageInfoByKey(0, 2,
		// 2 + "", UserMessageType.userSysMessage.getValue(), 0, 50);
		PageData pageData = MessageClient.getInstance().pageMessageInfoByKey(0,
				2, 61 + "", UserMessageType.userCommentReply.getValue(), 0, 50);

		if (pageData != null && pageData.getContent() != null
				&& pageData.getContent() instanceof List) {
			List<MessageInfo> messageInfos = (List<MessageInfo>) pageData
					.getContent();
	...
	public void listOnlineGame() {
		int appParamId = 927;
		String cacheKey = EGameCacheKeyV2
				.listOnlineGameServiceInfoByAppId(appParamId);
		List<OnlineGameServiceInfo> result = getCacheList().getListT(
				OnlineGameServiceInfo.class, cacheKey);
		if (result != null) {
	...
	// A.参赛总人数(所有游戏实际参与人数*20+（1-100）的随机数)
	// EGameCacheKeyV2.getMatchTotalAttendNum()
	// B.各个赛种参赛人数(所有游戏实际参与人数*20+（1-100）的随机数)
	// EGameCacheKeyV2.getMatchCategoryAttendNum(categoryId)
	// C.每个比赛的参赛人数(实际参与人数*20+（1-100）的随机数)
	// EGameCacheKeyV2.getMatchAttendNum(matchId)
	...
	//
	// // long matchTotalAttendNum =
	// Utils.toLong(getGameCache().get(EGameCacheKeyV2.getMatchTotalAttendNum()),
	// 0);
	...
	public void listMatchCheatInfoFromRMI() throws RemoteException {
		...
		List<String> result = EgameMatchClient.getInstance()
				.listAllCheatPackageNames(0, 0);
	...
	public void getMatchCheatInfoFromRMI() {
		...
		try {
			result = EgameMatchClient.getInstance().getMatchCheatInfoById(0, 0,
					id);
		} catch (RemoteException e) {
	...
	public void getMatchCheatInfoFromCache() {
		...
		MatchCheatInfo result = getGameCache().getT(MatchCheatInfo.class,
				cacheKey);
		System.out.println(result);
	}
	...
	public void setMatchCheatInfoFromRMI() {
		...
		getGameCache().set(EGameCacheKeyV2.getMatchCheatInfoById(info.getId()),
				info);
	}
	...
	public void getMatchScoreManagerByUserIdAndStageId() {
		...
		int stageId = 110;
		String cacheKey = EGameCacheKeyV2
				.getMatchScoreManagerByUserIdAndStageId(userId, stageId);
		MatchScoreManagerInfo result = getGameCache().getT(
				MatchScoreManagerInfo.class, cacheKey);
		System.out.println(result);
	}
	...
	public void getMatchGamePoolInfoById() {
		...
		MatchGamePoolInfo result = getGameCache().getT(MatchGamePoolInfo.class,
				cacheKey);
		System.out.println(result);
	}
	...
	public void test() throws RemoteException {
		...
		String cacheKey = EGameCacheKeyV2
				.getFlowChannelInfoByChannelCode(channelCode);
		FlowChannelInfo result = getFlowCache().getT(FlowChannelInfo.class,
				cacheKey);
		System.out.println(result.getCallbackUrl());

		// FlowChannelInfo flowChannelInfo =
		// EgameFlowClient.getInstance().getFlowChannelInfoByChannelCode(ConstVar.APP_FLOW,
		// 0, "700750");
		// if (flowChannelInfo == null ||
		// Utils.stringIsNullOrEmpty(flowChannelInfo.getCallbackUrl())) {
