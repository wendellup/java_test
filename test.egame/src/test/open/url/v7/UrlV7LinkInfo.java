...
public class UrlV7LinkInfo implements Serializable {
	...
	public UrlV7LinkInfo(String line, String prefixUrl,
		...
		for (String paramKey : extraParams.keySet()) {
			String paramVal = extraParams.get(paramKey);
			extraParamStr = extraParamStr + paramKey + "=" + paramVal + "&";
		}
		extraParamStr = extraParamStr.substring(0, extraParamStr.length() - 1);
		strs[2] = strs[2].trim();
		if (strs[2].indexOf("?") != -1) {
			strs[2] = strs[2] + "&" + extraParamStr;
		} else {
			strs[2] = strs[2] + "?" + extraParamStr;
		}
		method = strs[0].trim();
		desc = strs[1].trim();
		orgUrl = prefixUrl + strs[2];
		String natureUrl = prefixUrl + strs[2];
